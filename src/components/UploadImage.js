...
function UploadImage(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await fetch(IMAGE_PREDICTION_ENDPOINT, {
      method: "POST",
      body: formData,
      headers: {
    ...
    if (prediction !== null) {
      ...
      props.onInvalidImage(
        "Looks like your image did not return any results. Please try another one. Accepted formats are .jpg, .jpeg or .png"
      );
    }
    console.log(json);
  };
  return (
    <div className="container d-flex justify-content-center">
      <form className="fw-bold" onSubmit={handleSubmit}>
        <div className="mb-3">
          <label htmlFor="imageUpload" className="form-label text-dark">
            <h4>Find cars by uploading an image</h4>
          </label>
          <input
            type="file"
